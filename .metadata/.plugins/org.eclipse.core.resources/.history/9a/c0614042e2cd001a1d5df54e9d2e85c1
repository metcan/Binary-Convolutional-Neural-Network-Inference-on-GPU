...
__global__ void compK_matrix(T* InputImageData, T kernel_value,
    T* outputImageData, const int channel_in, const int width, const int height, const  int weight_row, const int weight_col) {

    float accum;
    int col = threadIdx.x + blockIdx.x * blockDim.x;   //col index
    int row = threadIdx.y + blockIdx.y * blockDim.y;   //row index
    int maskRowsRadius = weight_row / 2;
    int maskColsRadius = weight_col / 2;
    ...
    for (int k = 0; k < channel_in; k++) {      //cycle on kernel channels
        if (row < height && col < width) {
            ...
            for (int i = 0; i < weight_row; i++) { //cycle on mask rows

                for (int j = 0; j < weight_col; j++) { //cycle on mask columns
...
void xnor_convolution(matrix3d<float> &h_input_tensor, matrix4d<unsigned int> &h_weight_tensor, matrix3d<float> &h_output_tensor, const float alpha, int kernel_row, int kernel_col, bool padding=true)
{
	...
	compK_matrix<float><<<grid_size, block_size, 0, stream1>>>(d_input_tensor.arr, kernel_value,
		d_K_matrix.arr, d_input_tensor.channel, d_input_tensor.col, d_input_tensor.row, h_weight_tensor.row, h_weight_tensor.col);
	//
	scalar_multiplication<float><<<grid_size, block_size, 0, stream1>>>(d_K_matrix.arr, alpha, d_K_matrix.row, d_K_matrix.col);
